...
internal object TimestampConverter : LogicalTypeConverter<Long, Long> {
    override val name = io.debezium.time.Timestamp.SCHEMA_NAME

    private class AvroReader(sourcePrecision: TimePrecision) :
        KafkaValueReaders.TimestampReader<Long>(sourcePrecision, MILLIS) {
        override fun deserialize(ts: Long, reuse: Any?): Long = ts
    }

    private class AvroWriter(targetPrecision: TimePrecision) :
        KafkaValueWriters.TimestampWriter<Long>(MILLIS, targetPrecision) {
        override fun serialize(ts: Long): Long = ts
    }

    private class ParquetReader(sourcePrecision: TimePrecision, desc: ColumnDescriptor) :
        KafkaParquetReaders.TimestampReader<Long>(sourcePrecision, MILLIS, desc) {
        override fun deserialize(ts: Long, reuse: Any?): Long = ts
    }

    private class ParquetWriter(targetPrecision: TimePrecision, desc: ColumnDescriptor) :
        KafkaParquetWriters.TimestampWriter<Long>(MILLIS, targetPrecision, desc) {
        override fun serialize(ts: Long): Long = ts
    }

    override fun toIcebergType(idSupplier: Supplier<Int>, schema: Schema): Type = TimestampType.withoutZone()

    override fun toIcebergValue(sValue: Long): Long = MICROS.convert(sValue, MILLIS)
