...
public class ServiceClientMetricCollector implements ClientMetricCollector {
    ...
    public void collect(MetricCollection metricCollection) {
        ...
        var metrics = this.metrics.computeIfAbsent(
            operationName,
            name -> {
                OperationMetrics cmt = new OperationMetrics(name);
                cmt.bindTo(registry);
                return cmt;
            }
        );
    ...
    public void close() {
        logger.info("Close service client metric collector of service {}", serviceId);
        metrics.values().forEach(metrics -> metrics.unbindFrom(registry));
        metrics.clear();
    }
    ...
    private class OperationMetrics implements MeterBinder {
        ...
        synchronized public void bindTo(@NotNull MeterRegistry registry) {
            ...
            this.durationTimer = Timer.builder(String.format("%s.api.call.duration", prefix))
                .description("API call duration timing")
